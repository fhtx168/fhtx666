...
用法:
    python3 nami_red_cloaked_uncle_technology_talkshow.py --flow-id 701 -p "用户需求描述"
    python3 nami_red_cloaked_uncle_technology_talkshow.py --flow-id 701 -p "需求" --file '[]'
"""
...
def print_feishu_error(error_msg):
    error_card = {
        ...
        "header": {
            "title": {"tag": "plain_text", "content": "❌ 纳米 红衣大叔谈科技 beta 执行出错"},
            "template": "red"
        },
        "elements": [
            {
...
def print_channel_error(channel: str, error_msg: str):
    ...
    if is_feishu_channel(channel):
        ...
        print(f"❌ 纳米 红衣大叔谈科技 beta 执行出错：{error_msg}")
...
def main():
    parser = argparse.ArgumentParser(description="纳米 红衣大叔谈科技 beta")
    ...
    if args.verbose:
        print_feishu_progress("纳米 红衣大叔谈科技 beta 完成！")
