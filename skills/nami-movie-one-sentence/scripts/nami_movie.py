...
用法:
    python3 nami_movie.py --flow-id 701 -p "用户需求描述"
    python3 nami_movie.py --flow-id 701 -p "需求" --file '[]'
"""
...
def print_feishu_error(error_msg):
    error_card = {
        ...
        "header": {
            "title": {"tag": "plain_text", "content": "❌ 纳米 一句话生成大片 执行出错"},
            "template": "red"
        },
        "elements": [
            {
...
def print_channel_error(channel: str, error_msg: str):
    ...
    if is_feishu_channel(channel):
        ...
        print(f"❌ 纳米 一句话生成大片 执行出错：{error_msg}")
...
def call_nami_api(
    ...
    # 启动进度提示
    progress = ProgressPrinter(interval=PROGRESS_INTERVAL, message="正在生成大片，请耐心等候")
    progress.start()
...
def main():
    parser = argparse.ArgumentParser(description="纳米 一句话生成大片 - 飞书优化版")
    ...
    if args.verbose:
        print_feishu_progress("纳米 一句话生成大片 完成！")
