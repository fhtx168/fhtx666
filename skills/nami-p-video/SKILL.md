...
name: nami-p-video
description: |
  纳米 P视频。根据用户提供的一段详细创意描述与可选附件，生成充满惊喜的短视频。
  触发场景：
  (1) 用户说"生成视频"、"做个视频"
  (2) 用户说"短视频"、"创意视频"
  (3) 用户上传图片要求"生成视频"
...
根据用户提供的一段详细创意描述与可选附件，生成充满惊喜的短视频。

## 快速开始

```bash
# 先发送开始提示
message 发送 "收到！📹 正在生成视频，任务执行时间较长，请稍候..."

# 基本用法
python3 scripts/nami_p_video.py -p "生成一个30秒的广告视频"

# 飞书渠道需要加 --channel feishu 才会输出卡片
python3 scripts/nami_p_video.py -p "需求" --channel feishu
...
```json
{
  ...
  "header": {
    "title": {"tag": "plain_text", "content": "📹 纳米 P视频 完成"},
    "template": "blue"
  },
  "elements": [
