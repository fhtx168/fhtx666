...
用法:
    python3 nami_digital_human_v3.py --flow-id 2721 -p "用户需求描述"
    python3 nami_digital_human_v3.py --flow-id 2721 -p "需求" --file '[]'
"""
...
def print_feishu_error(error_msg):
    error_card = {
        ...
        "header": {
            "title": {"tag": "plain_text", "content": "❌ 纳米 数字人 3.0 执行出错"},
            "template": "red"
        },
        "elements": [
            {
...
def print_channel_error(channel: str, error_msg: str):
    ...
    if is_feishu_channel(channel):
        ...
        print(f"❌ 纳米 数字人 3.0 执行出错：{error_msg}")
...
def call_nami_api(
    ...
    # 启动进度提示
    progress = ProgressPrinter(interval=PROGRESS_INTERVAL, message="正在生成数字人视频，请耐心等候")
    progress.start()
...
def main():
    parser = argparse.ArgumentParser(description="纳米 数字人 3.0 - 飞书优化版")
    ...
    if args.verbose:
        print_feishu_progress("纳米 数字人 3.0 生成完成！")
